/**
 * File: g:\vue\jswei.cn\src\components\public\marquee.vue
 * Created Date: 2017-12-02 11:07:52
 * Author: 魏巍
 * -----
 * Last Modified: 魏巍
 * Modified By: 2017-12-02 11:58:36
 * -----
 * Copyright (c) 2017 魏巍
 * ------
...
<template>
    <div :style="{width:scrollWidth}" ref="marquee" class="marquee">
        <ul :style="{transition:transition}" ref="marqueelist" class="marquees fix" :data-timerId="timerId">
            <li v-for="(item,index) in list" :key="'m'+index" class="marquee-item">{{item.title}}<span>{{item.date}}</span></li>
        </ul>
    </div>
</template>
